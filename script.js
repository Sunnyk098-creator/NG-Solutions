...
async function processSignupStep1() {
    ...
    try {
        ...
        pendingSignupUser = { 
            name, email, password: pass, pin, tgUserId: telegram, isBanned: false, balance: 0, 
            apiKey: generateApiKey(), botAlerts: true, memberSince: joinDate
        }; 
...
async function syncData() {
    ...
    try {
                ...
                if (lastSeenPostTimestamp < latestPost.timestamp) {
                    ...
                    if(popup) {
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => { popup.classList.add('hidden'); popup.classList.remove('flex', 'opacity-0'); }, 500);
                        }, 3000);
                    }
                }
            }
        ...
        console.error("Sync Process Issue:", e);
    }
}
...
function markPostsAsRead() {
    ...
    let popup = document.getElementById('new-message-popup');
...
function updateProfileDashboardUI() {
    ...
    if (pLblCustom) {
        if (currentUser.customId) {
            ...
            pLblCustom.className = "font-black text-sm text-blue-500 font-mono";
        } else {
        ...
        img.onload = function () {
            ...
            apiCall('UPDATE_DP', { phone: currentUser?.phone, dp: base64Data })
                .then(() => {
                    ...
                    updateUI(); updateProfileDashboardUI(); showToast("Profile picture updated!");
...
function redirectToTelegramChannel() {
    let link = (globalSettings && globalSettings.channelUrl) ? globalSettings.channelUrl : "https://t.me/NG_SOLUTION";
    ...
    return new Promise(resolve => {
        ...
        if(rocketOverlay) {
            ...
            setTimeout(() => {
                ...
                showResultBox(data, true, resultOverlay, resolve);
            }, 2000); 
...
function showActionError(data) {
    return new Promise(resolve => {
        const resultOverlay = document.getElementById('txn-result-overlay');
        showResultBox(data, false, resultOverlay, resolve);
    });
}

function showResultBox(data, isSuccess, resultOverlay, resolve) {
    if(!resultOverlay) return resolve();
    let bgIcon = document.getElementById('txn-result-icon-bg');
    let icon = document.getElementById('txn-result-icon');
    let title = document.getElementById('txn-result-title');
    let errorBox = document.getElementById('txn-result-error-box');
    ...
    if(isSuccess) {
        ...
        title.className = "text-xl font-bold text-gray-800 mb-2 tracking-wide animate-[slideUpFade_0.5s_ease-out_0.1s] opacity-0";
        ...
        icon.className = "fas fa-times animate-shake";
        title.innerText = 'Payment Failed';
        title.className = "text-xl font-bold text-red-600 mb-2 tracking-wide animate-[slideUpFade_0.5s_ease-out_0.1s] opacity-0";
        if(errorBox) {
    ...
    if(data.dp && isSuccess) {
        ...
        dpInitial.innerText = data.name ? data.name.charAt(0).toUpperCase() : (isSuccess ? 'U' : 'X');
    }
...
async function processSend() {
    ...
    try {
        ...
        let txn = createTxnObj('out', 'Sent to ' + name, amt, 'Success', 'fa-paper-plane', 'blue', name, sendResolvedPhone);
        txn.comment = comment;
...
async function processScanPay() {
    ...
    try {
        ...
        playSound('debit');
        sendTelegramMsg(currentUser?.tgUserId, formatTgMsg('out', 'Scanned Payment Sent to ' + name, amt, `TXN: ${txn.id}`)); 
...
async function processAdd() {
    ...
    try {
        ...
        const btn = document.getElementById('btn-upload-screenshot');
        if (btn) { btn.innerHTML = `<i class="fas fa-image"></i> Upload Screenshot`; btn.className = "w-full mb-4 py-3 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border border-gray-200 bg-gray-50 text-gray-600"; }
...
async function processLifafaCreate() {
    ...
    if(type === 'standard' || type === 'coin') {
        ...
        minAmount = parseFloat(document.getElementById('lif-min-amt').value); maxAmount = parseFloat(document.getElementById('lif-max-amt').value);
...
async function renderMyLifafas() {
    ...
    try {
        let res = await fetch(`https://ng-solutions-646d2-default-rtdb.firebaseio.com/lifafas.json?orderBy="createdBy"&equalTo="${currentUser.phone}"`);
    ...
    try {
        ...
        if (details.alreadyClaimed) {
            if (details.referActive) {
                document.getElementById('lif-public-step-1').classList.add('hidden'); document.getElementById('lif-public-refer-step').classList.remove('hidden');
                document.getElementById('public-lif-refer-link').value = `https://${window.location.host}/?lifafa=${currentLifafaId}&ref=${lifafaClaimerPhone}`; return;
...
function renderLifafaResult(data) {
    ...
    if(data.type === 'scratch') animHtml = `<div class="w-32 h-32 mx-auto bg-gray-200 rounded-xl overflow-hidden relative anim-scratch-reveal mb-4 border-2 border-gray-300 shadow-inner"><div class="absolute inset-0 bg-gradient-to-br from-blue-300 to-blue-500 flex flex-col items-center justify-center font-black text-3xl text-white shadow-xl"><span class="text-[10px] uppercase tracking-widest text-blue-100 mb-1">You Won</span>₹${data.amount}</div></div>`;
        ...
        let coinClass = data.amount > 0 ? 'bg-gradient-to-br from-blue-400 to-blue-600 text-white' : 'bg-gradient-to-br from-gray-200 to-gray-400 text-gray-600'; let coinText = data.amount > 0 ? `₹${data.amount}` : `0`; let coinTitle = data.amount > 0 ? 'You Won!' : 'Bad Luck!';

...
let isBalanceVisible = true; // Default to visible for the new layout
...
function updateApiKeyUI() {
    ...
    if(elUrlUpi) elUrlUpi.innerHTML = `https://${domain}/api/upi.php?token=<span class="accent-text">${key}</span>&upi_id=<span class="text-green-500">{upi_id}</span>&amount=<span class="text-green-500">{amount}</span>&comment=<span class="text-green-500">{comment}</span>`;
...
function togglePrivacyMode() {
    ...
    apiCall('UPDATE_PRIVACY', { phone: currentUser?.phone, privacyMode: isEnabled });
    if(currentUser) currentUser.privacyMode = isEnabled; 
    isBalanceVisible = !isEnabled;
...
function openCustomIdModal() { 
    document.getElementById('custom-id-pricing').innerText = "Cost: ₹5"; 
    document.getElementById('customIdModal').classList.remove('hidden'); 
    setTimeout(()=>document.getElementById('customIdModal').classList.remove('opacity-0'), 10); 
}

function closeCustomIdModal() { 
    document.getElementById('customIdModal').classList.add('opacity-0'); 
    setTimeout(()=>document.getElementById('customIdModal').classList.add('hidden'), 300); 
}
...
function updateProfileDashboardUI() {
    ...
    if (pName) {
        pName.innerHTML = currentUser.name;
    }
...
function initApp() {
    if(currentUser) {
        ...
        if(document.getElementById('ui-user-phone')) document.getElementById('ui-user-phone').innerText = currentUser.customId || currentUser.phone;
        
        // Detailed Card Elements (matching screenshot layout)
...
async function verifyLifafaUser() {
    ...
    try {
        ...
        if(!user) throw new Error("User not found in NG SOLUTION!");
...
function renderLifafaResult(data) {
    ...
    if(data.type === 'scratch') {
        ...
        let coinClass = data.amount > 0 ? 'bg-gradient-to-br from-blue-400 to-blue-600 text-white' : 'bg-gradient-to-br from-gray-200 to-gray-400 text-gray-600';
...
async function processKeeperLock() {
    ...
    let txn = createTxnObj('out', 'Locked in Keeper', amt, 'Success', 'fa-lock', 'blue', 'Self Vault', 'N/A');
...
function updateStatsDashboard() {
    ...
    // Home Page specific elements for UI requirements
    ...
    let hNumTxn = document.getElementById('home-stats-txns'); if(hNumTxn) hNumTxn.innerText = totalTxns;
